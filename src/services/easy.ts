...
export async function getIssues(
  ...
  try {
    ...
    const issuesToGetParents: Issues[] = request.data.issues;

    const issuesWithParents = await Promise.all(
...
export async function removeIssueFromGroup(group_id: number, issue_id: number) {
  try {
    const group = (await api.get(`/groups/${group_id}.json`)).data
      .group as Group;

    const workedIssues = group.custom_fields?.find((e) => e.id === 130)
...
export async function getIssuesFromGroupUser(
  ...
  try {
    ...
    const issuesIdsFromGroupUser = filter2ndElementFrom3PartsArray(
    ...
    const issuesFromGroupUser = issues.filter((e) =>

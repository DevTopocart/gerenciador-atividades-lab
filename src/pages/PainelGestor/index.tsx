...
export default function PainelGestor() {
  ...
  async function fetchSubordinatesAndIssues() {
    ...
    try {
      ...
      const groupsSupervised: GroupsWithIssues[] = allGroups?.filter(
        (e) => e.custom_fields?.find((e) => e.id === 124)?.value === user.id,
      );
      const usersSupervised: UsersWithIssues[] = allUsers?.filter(
        (e) => e.supervisor_user_id === user.id || e.id === user.id,
      );

      const issuesIdsCurrentlyWorkedByGroups = new Set();

      groupsSupervised?.forEach((e, i, a) => {
        const issuesIds = filter2ndElementFrom3PartsArray(
          e.custom_fields!.find((e) => e.id === 130)?.value as Array<string>,
        );
      ...
      const issuesFromSubordinates = await getAllIssuesFromSubordinates(
        [
          ...usersSupervised!.map((e) => e.id),
          ...groupsSupervised!.map((e) => e.id),
        ],
        0,
        25,
        [],
      ...
      if (!issuesFromSubordinates || issuesFromSubordinates.length === 0)
        toast.info("Nenhuma atividade encontrada");

      issuesFromSubordinates?.forEach((issue, i, a) => {
        groupsSupervised?.forEach((group) => {
          const issuesIdsFromGroup = filter2ndElementFrom3PartsArray(
            group.custom_fields?.find(
              (e) => e.id === 130 && e.field_format === "easy_lookup",
    ...
    if (group.issues?.map((e) => e.id).includes(issue.id)) {
      try {
        ...
        gestor.setGroups(
          gestor.groups?.map((e) =>
            e.id === group.id
              ? { ...e, issues: e.issues?.filter((e) => e.id !== issue.id) }
              : e,
          ),
        );
        toast.success(
          `A atividade ${issue.subject} foi removida do usuário ${group.name}`,
        );
      } catch (error) {
        toast.error(
          `Houve um erro ao remover a atividade ${issue.subject} do usuário ${group.name}`,
        );
      } finally {
        setLoading(false);
      }
      ...
      try {
        ...
        gestor.setGroups(
          gestor.groups?.map((e) =>
            e.id === group.id
              ? { ...e, issues: e.issues ? [...e.issues, issue] : [issue] }
              : e,
          ),
        );
        toast.success(
          `A atividade ${issue.subject} foi adicionada ao usuário ${group.name}`,
        );
      } catch (error) {
        toast.error(
          `Houve um erro ao adicionar a atividade ${issue.subject} ao usuário ${group.name}`,
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    ...
    >
        ...
        {issueToAssign && (
          <DialogContent>
            <Typography variant="h6">
              Selecione os usuários para atribuir à esta tarefa:
            </Typography>
            <Divider></Divider>
            {gestor.groups?.map((group, i) => (
              ...
              >
                ...
                <IconButton
                  onClick={(event) =>
                    handleTaskClickOnGroup(event, issueToAssign!, group)
                  }
                >
                  {group.issues?.map((e) => e.id).includes(issueToAssign.id) ? (
      ...
      >
        ...
        >
          <Typography variant="caption" gutterBottom>
            Logado como {user.firstname} {user.lastname}
          </Typography>
          &nbsp;
          ...
          gestor.users.length === 0 && (
            ...
            >
              ...
              >
                ...
                supervisor. Se for o caso, solicite a inclusão no EasyProject
                pela TI - Produção enviando um email para{" "}
        ...
        {gestor.groups &&
          ...
          gestor.groups.map((group, i) => (
            ...
            >
              ...
              >
                {gestor.issues &&
                  gestor.issues.length > 0 &&
                  gestor.issues
                    .filter((i) => filterIssuesBySearchKey(searchKey, i))
                    .sort((a, b) => {
                      return group.issues?.map((e) => e.id).includes(a.id)
                        ? -1
                        : 1;
                    })
                    .map((issue, i) => (
                      <CardPainelGestor
                        key={i}
                        isSelected={
                          group.issues?.map((e) => e.id).includes(issue.id)
                            ? true
                            : false
                        }
                        handleTaskClick={(e) =>
                          handleTaskClickOnGroup(e, issue, group)
                        }
                        issue={issue}
                      />
                    ))}
      ...
      <Typography variant="caption">
        DICA: Pressione <b>SHIFT</b> e selecione uma tarefa para atribui-la a
        multiplos usuários
      </Typography>
    </Box>
  );

...
export default function usePainelGestor() {
  ...
  const issuesFromGroups = groups
    ? groups
        ?.filter((group) => group.issues)
        .map((group) => group.issues)
        .flat()
    : ([] as Issues[]);
  const issuesFromUsers = users
    ? users
        ?.filter((user) => user.issues)
        .map((user) => user.issues)
        .flat()
    : ([] as Issues[]);

  const issues = [...issuesFromUsers, ...issuesFromGroups];
